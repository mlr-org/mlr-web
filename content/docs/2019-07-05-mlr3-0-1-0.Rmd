...
While mlr was only one big package that included everything starting from preprocessing, tuning, feature-selection to visualization, mlr3 is a **package framework** consisting of multiple packages which contain specific parts of the functionality required for a complete machine-learning workflow.

## Background - why a rewrite?

The addition of many features to mlr has led to a "feature creep" which made it hard to maintain and extend.  

Due to the many tests in mlr, a full CI run of the package on Travis CI takes more than 30 minutes.
